...
def overall(csv_file):
	with open(csv_file, "a", newline="") as myfile:
		...
		#scrapey(wr, "test")
		#wr.writerow(['date','awayteam','awayteamrank','hometeam','hometeamrank','percentbetaway','percentbethome','awayOpeningSpread','awayOpeningLine','homeOpeningSpread','homeOpeningLine','awayClosingSpread','awayClosingLine','homeClosingSpread','homeClosingLine','awayOpeningMoney','homeOpeningMoney','awayClosingMoney','homeClosingMoney'])
		year = 17
		month = 1
		day = 7
		while year < 19:
...
def scrapey(wr, yearly):
	website = 'https://www.sportsbookreview.com/betting-odds/ncaa-basketball/?date=' + yearly
	#website = 'https://www.sportsbookreview.com/betting-odds/ncaa-basketball/?date=20171228'
			...
			if not awayClosingSpread == None:
				if not winningTeam == None:
					if not winningScore == None:
						awayList = getMoneyLine(g)
						awayOpeningMoney = awayList[0]
						homeOpeningMoney = awayList[1]
						awayClosingMoney = awayList[2]
						homeClosingMoney = awayList[3]
						wr.writerow([yearly,awayteam,awayteamrank,hometeam,hometeamrank,percentbetaway,percentbethome,awayOpeningSpread,awayOpeningLine,homeOpeningSpread,homeOpeningLine,awayClosingSpread,awayClosingLine,homeClosingSpread,homeClosingLine, awayOpeningMoney,homeOpeningMoney,awayClosingMoney,homeClosingMoney])
						print('row written')	
...
def whoWon(g, awayteam, hometeam):
	...
	if winner == hometeam:
		loser = awayteam
	if winner != awayteam and winner != hometeam:
		return [None, None]
	output = [winner, loser]
	return output
